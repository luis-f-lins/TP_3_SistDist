...
bool disabled = false;
int leader_pid = -1, pid, max_id, sent_msgs_count, recv_msgs_count;
...
int keycommands(){ // Reads keypress at any time and prints 'Done!' if key is f, and ends program is key is g.
    ...
    while(inp!='g'){
        ...
        if (inp=='1'){
            ...
            printf("Sent messages : %d\nReceived messages : %d\nCurrent leader: %d\n", sent_msgs_count, recv_msgs_count, leader_pid);
        }
        else if(inp =='5'){
...
int checkLeader(){
    ...
    while(1){
        if(leader_pid < 0 && election_timer < 0){
        ...
        else if(leader_pid < 0 && election_timer > 0 && (clock() - election_timer) > 2/(pid^3)*10^2){
            ...
            }else if(election_timer > 0 && (clock() - election_timer) > 2/(pid^3)*10^2){
                printf("Starting new election \n");
                for(int i = 1; i <= max_id; i++){
                    if(i != pid)
                        sendMessages(1, (PORT + i));
